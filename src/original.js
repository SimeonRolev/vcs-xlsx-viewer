...
function excellToDayJS(numFmt) {
    return numFmt
        .toUpperCase()
        .replace(/H([,.:/\\-_|])MM/g, 'H$1mm')
        .replace(/H([,.:/\\-_|])M/g, 'H$1m')
        .replace(/MM([,.:/\\-_|])S/g, 'mm$1S')
        .replace(/M([,.:/\\-_|])S/g, 'm$1S')
        .replace(/S/g, 's')
        .replace(/N/g, 'd')
        .replace(/AM\/PM/g, 'A')
        .replace(/"/g, '')
...
function parseCellValue(value) {
    return isNumeric(value) ? parseFloat(parseFloat(value).toFixed(2)) : value;
}

function addImageToCell({ image, mediaImage, tdElement }) {
    const { tl, br } = image.range;
    console.log('Tl:', tl.row, tl.col, tl);
    console.log('Br:', br.row, br.col, br);
...
async function renderXlsx({
    ...
    const viewerMethods = {
                ...
                try {
                    ...
                    new ExcelJS.Workbook().xlsx
                        ...
                        .then((workbook) => {
                            ...
                            if (
                                viewerElements.tipElement &&
                                getPrototype(viewerElements.tipElement).indexOf(
                                    'element'
                                ) !== -1
                            ) {
                                viewerElements.tipElement.style.display =
                                    'none';
        ...
        createXlsxContainerElement() {
            ...
            const oldXlsxViewerContainerElement = xlsxElement.querySelector(
            ...
            if (oldXlsxViewerContainerElement) {
                viewerElements.xlsxElement.replaceChild(
                    viewerElements.containerElement,
                    oldXlsxViewerContainerElement
                );
            } else {
                viewerElements.xlsxElement.appendChild(
                    viewerElements.containerElement
                );
            }
        },
        createTableContainerElement() {
            for (let i = 0; i < viewerParams.sheetList.length; i++) {
                ...
                const xlsxViewerTableItemElement =
                    document.createElement('div');
                xlsxViewerTableItemElement.classList.add(
                    'xlsx-viewer-table-content'
                );
                viewerMethods.createTableContentElement(
                    sheetItem,
                    xlsxViewerTableItemElement
                );
                viewerElements.tableElement?.appendChild(
                    xlsxViewerTableItemElement
                );
            }
        },
        createTableContentElement(sheetItem, xlsxViewerTableItemElement) {
            // set table element
            ...
            const appendTrElementToTbodyElement = (currentPage = 0) => {
                requestAnimationFrame(() => {
                    ...
                    if (
                        currentPage * frameRenderSize <
                        tbodyTrElementArr.length
                    ) {
            ...
            if (sheetItem.columns.length > 0) {
                ...
                for (let i = 0; i < sheetItem.columns.length; i++) {
                    ...
                    const columnWidth =
                        column.width > 0 ? column.width / 0.125 : 100;
            ...
            if (sheetItem.rows.length > 0) {
                for (let i = 0; i < sheetItem.rows.length; i++) {
                    ...
                    for (let j = 0; j < sheetItem.columns.length; j++) {
                        ...
                        if (
                            cell.isMerged &&
                            cell.master._address !== cell._address
                        ) {
                            continue;
                        }
                        ...
                        if (
                            ...
                            const merge = sheetItem.merges.find(
                                (item) => item.address === cell._address
                            );
                            if (merge) {
                                const maxCol = Math.max.apply(
                                    Math,
                                    merge.cells.map((cell) => cell.col)
                                );
                                const maxRow = Math.max.apply(
                                    Math,
                                    merge.cells.map((cell) => cell.row)
                                );
                        ...
                        if (cell.style?.fill) {
                            ...
                            tdElement.style.backgroundColor = fgColor?.argb
                                ? utilMethods.parseARGB(fgColor?.argb)?.color
                                : '#fff';
                        }
                        // set cell border
                        if (cell.style?.border) {
                            const { top, bottom, left, right } =
                                cell.style.border;
                            tdElement.style.borderTop = top?.color?.argb
                                ? '1px solid ' +
                                  utilMethods.parseARGB(top?.color?.argb)?.color
                                : '';
                            tdElement.style.borderBottom = bottom?.color?.argb
                                ? '1px solid ' +
                                  utilMethods.parseARGB(bottom?.color?.argb)
                                      ?.color
                                : '';
                            tdElement.style.borderLeft = left?.color?.argb
                                ? '1px solid ' +
                                  utilMethods.parseARGB(left?.color?.argb)
                                      ?.color
                                : '';
                            tdElement.style.borderRight = right?.color?.argb
                                ? '1px solid ' +
                                  utilMethods.parseARGB(right?.color?.argb)
                                      ?.color
                                : '';
                        }
                        // set cell font
                        if (cell.style?.font) {
                            ...
                            tdElement.style.color = color?.argb
                                ? utilMethods.parseARGB(color?.argb)?.color
                                : '#333';
                            tdElement.style.fontFamily = name;
                            tdElement.style.fontSize = size
                                ? `${size / 0.75}px`
                                : '14px';
                        ...
                        sheetItem.images
                            .filter(
                                (image) =>
                                    image.range.tl.nativeCol + 1 === cell.col &&
                                    image.range.tl.nativeRow + 1 === cell.row
                            )
                            .forEach((image) => {
                                const mediaImage =
                                    sheetItem.workbook.model.media.find(
                        ...
                        if (cell.style?.alignment?.textRotation) {
                            ...
                            content.style.transform = `rotate(${-cell.style
                                .alignment.textRotation}deg)`;
                        }
                        ...
                        if (getPrototype(cell.value) === 'object') {
                            ...
                            if (
                                ...
                                for (const span of richText) {
                                    ...
                                    if (span?.font) {
                                        ...
                                        spanElement.style.color = color?.argb
                                            ? utilMethods.parseARGB(color?.argb)
                                                  ?.color
                                            : '#333';
                            ...
                            content.innerText = Dayjs(cell.value).format(
                                excellToDayJS(cell.numFmt)
                            );
                        } else {
    ...
    if (
        ...
        viewerElements.tipElement &&
        getPrototype(viewerElements.tipElement).indexOf('element') !== -1
    ) {
    ...
    if (['blob', 'file'].includes(getPrototype(xlsxData))) {
        viewerParams.arrayBuffer = await utilMethods.blobOrFileToArrayBuffer(
            xlsxData
        );
